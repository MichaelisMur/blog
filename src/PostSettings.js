...
export default class PostSettings extends React.Component{
    ...
    restore(){
        let fun = (refreshFunction) => {
            fetch("http://localhost:3001/restore", {
                method: 'POST',
                body: JSON.stringify({
                    id: this.props.id,
                    access_token: cookies.get("access_token"),
        ...
        return(
            <div className="PostSettingsContainer">
                <Link to={`/edit/${this.props.id}`}>
                    <img src="http://localhost:3001/public/settings.png" alt="del"
                        style={{display: cookies.get("admin")?"block":"none"}}
                    />
                </Link>
                <img src={this.state.shown ? "http://localhost:3001/public/delete.png" : "http://localhost:3001/public/restore.png"} alt="del"
                    style={{display: cookies.get("admin")?"block":"none"}}
